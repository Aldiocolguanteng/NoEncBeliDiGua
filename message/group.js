...
module.exports = welcome = async (kirbotz, anu) => {
		...
		try {
            ...
            kirbotz.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot, Tolong Jadikan Admin Ya, Ketik !menu', 'conversation')
             ...
             if (!welkom.includes(anu.jid)) return
              ...
              buff = await getBuffer(pic)
               kirbotz.sendMessage(mdata.id, { contentText: `${teks}`, footerText: `Welcome Message By ${setting.botName}`, buttons: [{buttonId: `!selamatdatang`,buttonText:{displayText: 'WELCOME BWANG'},type:1},{buttonId: `!rules`,buttonText:{displayText:'Rules Bot'},type:1},{buttonId:`!infogrup`,buttonText:{displayText:'Info Group'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
		}
            ...
            if (!welkom.includes(anu.jid)) return
               ...
               buff = await getBuffer(pic)
               kirbotz.sendMessage(mdata.id, { contentText: `${out}`, footerText: `Welcome Message By ${setting.botName}`, buttons: [{buttonId: `!bay`,buttonText:{displayText: 'BYE KAK👋'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
            }
		} catch (e) {
